...
export async function seedCatalog(prisma: PrismaClient) {
  ...
  const allQuestions = [
    ...
    },
    {
      ...
      type: 'technical',
      questionType: 'written',
      prompt: 'Describe the ACID properties in relational database transactions. Provide a practical web application scenario where an Isolation anomaly could cause business failure.',
      optionsJson: JSON.stringify([]),
      weight: 2.0,
      expectedAnswerRubric: 'Atomicity, Consistency, Isolation, Durability, banking/e-commerce inventory concurrency, transaction isolation levels',
    },
    ...
    },
    {
      ...
      type: 'technical',
      questionType: 'written',
      prompt: 'Design a scalable Rate Limiter service for a public REST API handling 100,000 requests/sec. Discuss the Token Bucket vs Sliding Window Log algorithms and Redis data structures used.',
      optionsJson: JSON.stringify([]),
      weight: 2.5,
    ...
    },
    {
      ...
      type: 'technical',
      questionType: 'written',
      prompt: 'Explain the Bias-Variance Tradeoff in machine learning. How do high bias and high variance manifest in training vs validation curves, and what techniques alleviate each?',
      optionsJson: JSON.stringify([]),
      weight: 2.0,
    ...
    },
    {
      ...
      type: 'technical',
      questionType: 'written',
      prompt: 'Describe the core architecture of Retrieval-Augmented Generation (RAG). How do vector embeddings, chunking strategies, and re-ranking improve LLM generation accuracy?',
      optionsJson: JSON.stringify([]),
      weight: 2.5,
      expectedAnswerRubric: 'Vector embeddings, chunking overlap, cosine similarity retrieval, vector database, cross-encoder re-ranking, mitigating hallucinations',
    },
