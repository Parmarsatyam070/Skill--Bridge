...
export const InterviewFeedbackCard: React.FC<InterviewFeedbackCardProps> = ({ evaluation }) => {
  const getReadinessBadge = (tier: string) => {
    switch (tier) {
      ...
      case 'ALMOST_READY':
        return { label: 'Almost Ready', color: 'bg-teal-500/15 text-teal-300 border-teal-500/30' };
      case 'DEVELOPING':
  ...
  const getRecommendationBadge = (rec: string) => {
    switch (rec) {
      ...
      case 'RECOMMEND':
        return { label: 'Positive Signal', color: 'bg-teal-500/20 text-teal-300 border-teal-500/40' };
      case 'MAYBE':
        return { label: 'Mixed Signal', color: 'bg-amber-500/20 text-amber-300 border-amber-500/40' };
      case 'DO_NOT_RECOMMEND':
      default:
        return { label: 'Needs Additional Practice', color: 'bg-zinc-800 text-zinc-400 border-zinc-700' };
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* ── KPI HEADER CARD ────────────────────────────────────────── */}
      <div className="bg-[#111318] border border-[#2A2E38] rounded-2xl p-6 sm:p-8 shadow-xl relative overflow-hidden">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-[#1f242d]">
          ...
          <div className="flex items-center gap-4 sm:gap-6 shrink-0">
            ...
            <div className="text-center p-4 rounded-xl bg-[#1A1D24] border border-[#2A2E38] min-w-[110px]">
              <div className="text-3xl sm:text-4xl font-extrabold text-[#3aa398] font-mono">
                {evaluation.overallScore}
                <span className="text-xs text-zinc-500 font-sans font-normal">/100</span>
              </div>
              <span className="text-[11px] uppercase tracking-wider font-semibold text-zinc-400 mt-1 block">
                Advisory Score
              </span>
            </div>
            ...
            {evaluation.technicalScore !== undefined && (
              <div className="text-center p-4 rounded-xl bg-[#1A1D24] border border-[#2A2E38] min-w-[100px] hidden sm:block">
                <div className="text-2xl sm:text-3xl font-bold text-zinc-200 font-mono">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Strengths */}
        <div className="bg-[#111318] border border-[#2A2E38] rounded-2xl p-6 space-y-4 shadow-md">
      ...
      {evaluation.evidenceObserved && evaluation.evidenceObserved.length > 0 && (
        <div className="bg-[#111318] border border-[#2A2E38] rounded-2xl p-6 space-y-3 shadow-md">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
            {evaluation.evidenceObserved.map((ev, i) => (
              <div
                key={i}
                className="p-3 rounded-xl bg-[#1A1D24] border border-[#2A2E38] text-xs text-zinc-300 flex items-start gap-2"
              >
                <span className="text-[#2F8C82] font-mono text-xs font-bold mt-0.5">#{i + 1}</span>
                <span>{ev}</span>
              </div>
            ))}
          </div>
        ...
        <div className="bg-[#111318] border border-[#2A2E38] rounded-2xl p-6 space-y-4 shadow-md">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {evaluation.skillObservations.map((obs, i) => (
              <div
                key={i}
                className="p-3.5 rounded-xl bg-[#1A1D24] border border-[#2A2E38] space-y-1.5"
              >
                <div className="flex items-center justify-between text-xs">
                  <span className="font-semibold text-zinc-200">{obs.skill}</span>
                  <span className="font-mono text-[#3aa398] font-bold">{obs.rating}%</span>
                </div>
      ...
      {evaluation.recommendations && evaluation.recommendations.length > 0 && (
        <div className="bg-[#111318] border border-[#2A2E38] rounded-2xl p-6 space-y-3 shadow-md">
          <div className="flex items-center gap-2 text-[#2F8C82] font-semibold text-sm">
          ...
          <ul className="space-y-2">
            {evaluation.recommendations.map((rec, i) => (
              <li key={i} className="flex items-start gap-2.5 text-xs text-zinc-300">
                <span className="w-1.5 h-1.5 rounded-full bg-[#2F8C82] mt-1.5 shrink-0" />
                <span>{rec}</span>
              </li>
            ))}
          </ul>
