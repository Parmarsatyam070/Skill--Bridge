...
export const InterviewResponseInput: React.FC<InterviewResponseInputProps> = ({
  ...
  return (
    <div className="bg-[#0b1329] border border-[#1e293b] rounded-2xl p-5 sm:p-6 space-y-4 shadow-xl">
      <div className="flex items-center justify-between gap-3 text-xs text-slate-400">
        <label htmlFor="interview-answer-input" className="font-medium text-slate-300">
          Your Response:
        </label>
      ...
      <div className="relative">
        <textarea
          ...
          className="w-full bg-[#030712] border border-[#1e293b] rounded-xl p-4 text-sm text-slate-100 placeholder-slate-500 focus:outline-hidden focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all resize-y font-sans leading-relaxed disabled:opacity-50"
        />
      </div>
        ...
        <div className="text-xs text-slate-500">
          {charCount < minChars ? (
            ...
            <span className="text-slate-400 hidden sm:inline">
              Tip: Press <kbd className="px-1.5 py-0.5 rounded bg-[#0f172a] border border-[#1e293b] text-slate-300 font-mono text-[10px]">Ctrl+Enter</kbd> to submit
            </span>
          )}
        </div>
        ...
        <button
          ...
          className={`flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl font-medium text-sm transition-all duration-150 ${
            isValid && !isSubmitting
              ? 'bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-600 text-white shadow-lg shadow-blue-500/20 active:scale-[0.98]'
              : 'bg-[#0f172a] text-slate-500 cursor-not-allowed border border-[#1e293b]'
          }`}
        >
          {isSubmitting ? (
            <>
