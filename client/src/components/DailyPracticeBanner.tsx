...
export const DailyPracticeBanner: React.FC<DailyPracticeBannerProps> = ({ className = '', onStartSet }) => {
  ...
  if (completedToday) {
    return (
      <div
        className={`relative overflow-hidden rounded-2xl bg-[#0b1329] border border-emerald-500/30 p-4 sm:p-5 shadow-lg backdrop-blur-md ${className}`}
      >
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3.5">
            <div className="w-10 h-10 rounded-xl bg-[#0f172a] border border-emerald-500/40 flex items-center justify-center text-emerald-400 shrink-0">
              <CheckCircle2 className="w-5 h-5 text-emerald-400" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-[11px] font-mono uppercase font-semibold text-emerald-300 bg-emerald-500/10 border border-emerald-500/30 px-2 py-0.5 rounded">
                  Daily Protocol Completed
                </span>
                <span className="text-xs text-slate-400 flex items-center gap-1 font-mono">
                  <Flame className="w-3.5 h-3.5 text-amber-400 fill-amber-400" />
                  <strong className="text-white font-medium">{currentStreak} day{currentStreak !== 1 ? 's' : ''}</strong>
                </span>
              </div>
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-2xl bg-[#0b1329] border border-amber-500/30 p-4 sm:p-5 shadow-lg backdrop-blur-md ${className}`}
    >
      <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        {/* Left Side: Status Nudge & Copy */}
        <div className="flex items-start gap-3.5">
          <div className="w-10 h-10 rounded-xl bg-[#0f172a] border border-amber-500/40 flex items-center justify-center text-amber-400 shrink-0">
            <Flame className="w-5 h-5 text-amber-400 fill-amber-400" />
          </div>

          <div className="space-y-1">
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-[11px] font-mono uppercase font-semibold text-amber-300 bg-amber-500/10 border border-amber-500/30 px-2 py-0.5 rounded flex items-center gap-1.5">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full lg:w-auto shrink-0">
          {recommendedSet && (
            <div className="hidden xl:flex flex-col text-right pr-2">
              <span className="text-[10px] font-mono text-slate-400 uppercase">Target Set</span>
