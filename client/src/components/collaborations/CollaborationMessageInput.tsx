    ...
    <div className="space-y-2">
      ...
      <form onSubmit={handleSubmit} className="relative">
        <textarea
          ...
          rows={3}
          maxLength={maxChars}
          className="w-full bg-[#111318] border border-[#2A2E38] rounded-xl px-4 py-3 pb-10 text-sm text-[#F4F5F7] placeholder-[#8B90A0] focus:outline-none focus:border-[#2F8C82] focus:ring-1 focus:ring-[#2F8C82] transition-colors resize-none disabled:opacity-50 disabled:bg-zinc-900"
        ...
        <div className="absolute bottom-2.5 left-3 right-3 flex items-center justify-between pointer-events-none">
          <span
            className={`text-[11px] font-mono ${
              charCount > maxChars ? 'text-rose-400' : 'text-zinc-500'
            }`}
          >
            {charCount}/{maxChars}
          </span>
          ...
          <button
            ...
            className="pointer-events-auto px-3.5 py-1.5 rounded-lg bg-[#2F8C82] hover:bg-[#3aa398] disabled:bg-zinc-800 disabled:text-zinc-600 text-white text-xs font-medium flex items-center gap-1.5 transition-colors shadow-sm"
