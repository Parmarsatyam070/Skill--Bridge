...
const BridgeBotWidgetComponent: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      id: 'welcome',
      sender: 'assistant',
      text: `Hello ${user?.name || 'there'}! I'm **Bridge Bot**, your AI career navigator on SkillBridge. I am currently aligned to your domain: **${user?.studentProfile?.targetDomain || 'Full-Stack Web'}**.\n\nAsk me about your domain skill gaps, roadmap courses, or match score breakdowns!`,
  ...
  const handleSend = async (promptToSend?: string) => {
    ...
    try {
      ...
      }>('/ai/chat', { prompt: text, activeDomain });
      ...
      setMessages(prev => [
        ...prev,
        {
          id: 'error-' + Date.now(),
          sender: 'assistant',
          text: `⚠️ I encountered a temporary issue connecting to the skill intelligence engine. Please try asking again.`,
          timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        },
      ]);
    } finally {
    ...
    <div className="fixed bottom-3 right-3 sm:bottom-5 sm:right-5 z-50 font-sans gpu-accel">
      {!isOpen ? (
        ...
        >
          ...
          <span className="text-xs font-semibold tracking-wide">Bridge Bot</span>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h4 className="text-xs font-bold text-console-text">Bridge Bot AI</h4>
                  <select
                    value={activeDomain}
                    onChange={e => {
            ...
            {loading && (
              <div className="flex gap-2.5 items-center">
                ...
                <div className="bg-console-panel-raised border border-console-border rounded-2xl rounded-tl-none px-4 py-2.5 flex items-center gap-1.5">
                  ...
                  <span className="text-[11px] font-mono text-console-text-muted ml-2">Evaluating skill vectors...</span>
                </div>
              </div>
            )}
          ...
          <div className="p-3 bg-console-panel border-t border-console-border">
            ...
            >
              <input
                ...
                placeholder="Ask about skill gaps, courses, or matches..."
                disabled={loading}
