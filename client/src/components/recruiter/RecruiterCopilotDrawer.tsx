    ...
    <>
      ...
      {!propIsOpen && !isOpen && (
        <button
          ...
          className="fixed bottom-6 right-6 z-40 flex items-center gap-2.5 px-4 py-3 rounded-full bg-[#0b1329] border border-[#1e293b] hover:border-blue-500/50 text-white shadow-2xl hover:shadow-blue-500/20 transition-all transform hover:-translate-y-0.5 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
          aria-label="Open Recruiter Copilot Assistant"
        >
          <div className="relative flex items-center justify-center text-blue-400">
            <Bot className="w-5 h-5" />
            <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-emerald-400 rounded-full border-2 border-[#0b1329] animate-pulse" />
          </div>
          <div className="flex flex-col text-left">
            <span className="text-xs font-bold font-sans tracking-wide flex items-center gap-1.5">
              <span>Recruiter Copilot</span>
              <span className="text-[10px] font-mono px-1.5 py-0.2 rounded bg-blue-600/20 text-blue-400">
                AI
              </span>
            </span>
          </div>
      ...
      <div
        ...
        className={`fixed inset-y-0 right-0 z-50 w-full sm:w-[540px] md:w-[600px] bg-[#0b1329] border-l border-[#1e293b] shadow-2xl flex flex-col transition-transform duration-300 ease-out transform ${
          isOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
      >
        {/* Drawer Header */}
        <div className="p-4 sm:p-5 bg-[#0f172a] border-b border-[#1e293b] flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 id="recruiter-copilot-title" className="text-sm font-bold text-white tracking-tight">
                  Recruiter Copilot
                </h2>
                <span className="px-1.5 py-0.5 rounded text-[10px] font-mono font-semibold bg-amber-500/15 text-amber-400 border border-amber-500/30">
                  ADVISORY AI
                </span>
              </div>
              <p id="recruiter-copilot-desc" className="text-[11px] text-slate-400">
          ...
          <div className="flex items-center gap-1.5">
            {activeTab === 'chat' && (
              <button
                ...
                className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-[#0b1329] transition-colors"
            ...
            <button
              onClick={handleClose}
              className="p-2 rounded-lg text-slate-400 hover:text-white hover:bg-[#0b1329] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
              title="Close drawer"
        ...
        <div className="px-4 py-3 bg-[#0b1329] border-b border-[#1e293b] space-y-2.5 shrink-0">
          {/* Opportunity selector */}
          <div className="flex items-center justify-between gap-3 text-xs">
            <span className="text-slate-400 flex items-center gap-1.5 shrink-0 font-medium">
            ...
            <select
              ...
              className="flex-1 max-w-[340px] px-2.5 py-1 text-xs rounded-lg bg-[#0f172a] border border-[#1e293b] text-white focus:outline-none focus:border-blue-500 truncate"
          ...
          {/* Navigation Tabs */}
          <div className="flex rounded-lg bg-[#0f172a] p-1 border border-[#1e293b]" role="tablist">
            <button
              role="tab"
            ...
            >
              ...
              {selectedCandidateIds.length > 0 && (
                <span className="px-1.5 py-0.2 rounded-full text-[10px] font-mono bg-[#0b1329] text-white">
        ...
        <div className="px-4 py-2 bg-amber-500/10 border-b border-amber-500/20 flex items-start gap-2 text-[11px] text-amber-300 shrink-0">
        ...
        {rateLimitMessage && (
          <div className="px-4 py-2.5 bg-rose-500/15 border-b border-rose-500/30 flex items-start gap-2 text-xs text-rose-300 shrink-0">
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            >
                  ...
                  <div
                    className={`p-4 rounded-2xl text-xs leading-relaxed max-w-[92%] ${
                      m.role === 'user'
                        ...
                        : m.isError
                        ? 'bg-rose-500/15 border border-rose-500/30 text-white rounded-tl-xs'
                        : 'bg-[#0f172a] border border-[#1e293b] text-white rounded-tl-xs'
                    }`}
                  >
                    {m.isError ? (
                    ...
                    {m.supportingData && Array.isArray(m.supportingData) && m.supportingData.length > 0 && (
                      <div className="mt-3 pt-3 border-t border-[#1e293b] space-y-2">
                        <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 flex items-center justify-between">
                          ...
                          <span className="text-emerald-400">Verified Records</span>
                        </div>
                          ...
                          {m.supportingData.map((item: any, idx: number) => {
                            if (item.fullName) {
                              ...
                              return (
                                <div
                                  key={cand.id || idx}
                                  className="p-2.5 rounded-xl bg-[#0b1329] border border-[#1e293b] flex items-center justify-between gap-3 text-xs"
                                >
                                    ...
                                    {cand.skills && cand.skills.length > 0 && (
                                      <div className="flex flex-wrap gap-1 pt-1">
                                        {cand.skills.slice(0, 3).map((s, i) => (
                                          <span
                                            key={i}
                                            className="px-1.5 py-0.2 rounded text-[9.5px] bg-[#0f172a] text-slate-300 border border-[#1e293b]"
                                          >
                                            {s.name} ({s.score}%)
                                          </span>
                                        ))}
                            ...
                            if (item.skillName) {
                              ...
                              return (
                                <div
                                  key={idx}
                                  className="p-2 rounded-lg bg-[#0b1329] border border-[#1e293b] flex items-center justify-between text-xs"
                                >
                                  <span className="font-medium text-white">{item.skillName}</span>
                                  <div className="flex items-center gap-2 text-[11px] font-mono">
                                    <span className="text-blue-400">{item.demandCount} openings</span>
                                    <span className="text-emerald-400">({item.trendDirection})</span>
                                  </div>
                                </div>
                              );
                            }
                    ...
                    {m.role === 'assistant' && !m.isError && m.disclaimer && (
                      <div className="mt-3 pt-2 border-t border-[#1e293b]/50 text-[10px] text-slate-400 italic">
                        {m.disclaimer}
                      </div>
                    )}
                  </div>
              ...
              {queryMutation.isPending && (
                <div className="flex items-center gap-2 text-xs text-slate-400 p-3 rounded-xl bg-[#0f172a] border border-[#1e293b] max-w-xs">
            ...
            <div className="px-4 py-2 bg-[#0b1329] border-t border-[#1e293b] flex items-center gap-2 overflow-x-auto touch-scroll">
              ...
              {SUGGESTED_QUERIES.map((sq, i) => (
                <button
                  ...
                  className="px-2.5 py-1 text-[11px] rounded-full bg-[#0f172a] border border-[#1e293b] text-slate-400 hover:text-white hover:border-blue-500/50 whitespace-nowrap transition-colors shrink-0 disabled:opacity-50"
                >
                  {sq}
                </button>
              ))}
            ...
            <form
              ...
              className="p-3 bg-[#0f172a] border-t border-[#1e293b] flex items-center gap-2"
            >
              <input
                ref={inputRef}
                type="text"
                ...
                disabled={queryMutation.isPending}
                className="flex-1 px-3.5 py-2.5 text-xs rounded-xl bg-[#0b1329] border border-[#1e293b] text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 disabled:opacity-50"
                aria-label="Recruiter Copilot Query Input"
              />
              <button
                type="submit"
                disabled={!inputQuery.trim() || queryMutation.isPending}
                className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-600 text-white text-xs font-bold flex items-center gap-1.5 transition-colors disabled:opacity-40 disabled:cursor-not-allowed shadow-md shadow-blue-500/20"
        ...
        {activeTab === 'compare' && (
          <div className="flex-1 overflow-y-auto p-4 space-y-6 touch-scroll">
            ...
            <div className="space-y-2">
              ...
              {isLoadingApplicants ? (
                <div className="p-6 rounded-xl bg-[#0f172a] border border-[#1e293b] flex items-center justify-center gap-2 text-xs text-slate-400">
                  <Loader2 className="w-4 h-4 animate-spin text-blue-400" />
                  <span>Loading applicants for selected opportunity...</span>
                </div>
              ) : availableApplicants.length === 0 ? (
                <div className="p-6 rounded-xl bg-[#0f172a] border border-[#1e293b] text-center space-y-2">
                  <Users className="w-6 h-6 text-slate-400 mx-auto" />
                  <p className="text-xs font-semibold text-white">No Applicants Found for this Opportunity</p>
                  <p className="text-[11px] text-slate-400">
                    Select another role context above or wait for student applications to arrive.
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto p-1 bg-[#0b1329] rounded-xl border border-[#1e293b]">
                      ...
                      <button
                        ...
                        className={`p-2.5 rounded-lg border text-left transition-all flex items-center justify-between gap-2 ${
                          isSelected
                            ...
                            : 'bg-[#0f172a] border-[#1e293b] hover:border-slate-700 text-slate-300'
                        }`}
                      >
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <input
                            ...
                            checked={isSelected}
                            readOnly
                            className="w-3.5 h-3.5 rounded border-[#1e293b] text-blue-500 focus:ring-0 cursor-pointer pointer-events-none"
              ...
              <button
                ...
                className="w-full py-2.5 rounded-xl bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-600 text-white text-xs font-bold flex items-center justify-center gap-2 transition-colors disabled:opacity-40 disabled:cursor-not-allowed shadow-md shadow-blue-500/20"
              >
                {compareMutation.isPending ? (
                  <>
                    <Loader2 className="w-4 h-4 animate-spin" />
            ...
            {compareMutation.data?.comparison && (
              <div className="space-y-6 pt-2 border-t border-[#1e293b]">
                ...
                <div className="p-4 rounded-xl bg-[#0f172a] border border-[#1e293b] space-y-2">
                  ...
                  <p className="text-[10px] text-slate-400 italic pt-1 border-t border-[#1e293b]">
                        ...
                        >
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex items-center gap-2.5">
                              <span className="w-6 h-6 rounded-full bg-[#0b1329] border border-[#1e293b] text-xs font-mono font-bold flex items-center justify-center text-white">
                                #{idx + 1}
                              </span>
                              <div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 border-t border-[#1e293b]/60 text-xs">
                            {strengths.length > 0 && (
                              <div>
